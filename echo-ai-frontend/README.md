...
Create a `.env` file in the project root, or start from `.env.example`.
...
It now receives the Firebase web app config through the service worker registration URL so the same deployment env vars can drive both the app bundle and the push worker without manual file edits.

## ☁️ Production Deployment

For a shareable public app:

- deploy this frontend as the `echoai-frontend` static site on Render
- use the Render blueprint in the repo root as the source of truth
- build with `cd echo-ai-frontend && npm install && npm run build`
- publish `echo-ai-frontend/dist`
- set `VITE_API_URL` to the deployed backend URL
- keep `VITE_USE_MOCK=false`
- configure the Firebase web app env vars and `VITE_VAPID_KEY`
- set backend `ALLOWED_ORIGINS` to the exact frontend Render URL

The frontend URL is the only URL end users need. They should be able to create accounts, sign in, upload meetings, review dashboard data, and opt into browser push notifications directly from that URL.
